...
public class BbsDAO { //데이터베이스 관련 작업
  ...
  public BbsDTO read(int bbsno) {
    ...
    try {
      con=dbopen.getConnection();
      sql=new StringBuilder();
      sql.append(" SELECT bbsno, wname, subject, content, readcnt, passwd, grpno, indent, ansnum, ip, regdt");
      sql.append(" FROM tb_bbs ");
  ...
  public ArrayList<BbsDTO> list3(String col, String word, int nowPage, int recordPerPage){
	    ...
	    // 1 page : WHERE r>=1 AND r<=10 r=recordPerpage
	    ...
	    int endRow   = nowPage * recordPerPage;
	      ...
	      try{
		      ...
		      if(word.length()==0) { //검색을 하지 않는 경우
		        ...
		        //rownum 하면 옆에 새로운행번호 붙으면서 조건달수있음
		        sql.append("       FROM ( SELECT bbsno,subject,wname,readcnt,indent,regdt");
		        sql.append("              FROM tb_bbs");//grpno가높을수록 최신의 쓴거 그래서 내림차순
		        sql.append("              ORDER BY grpno DESC, ansnum ASC");
		        sql.append("           )");
		        sql.append("     )");
